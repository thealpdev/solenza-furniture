...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    if (categoryId) {
      ...
      // Scroll to products section to ensure user sees the change
  ...
  useEffect(() => {
    applyFilters()
  }, [selectedCategories, priceRange, inStock, preOrder, allProducts, sortBy, searchQuery])
  ...
  const applyFilters = () => {
    ...
    if (selectedCategories.length > 0) {
      filtered = filtered.filter(p =>
        p.category_id && selectedCategories.includes(String(p.category_id))
      )
    }
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(p =>
        (p.title_tr && p.title_tr.toLowerCase().includes(query)) ||
        (p.title_en && p.title_en.toLowerCase().includes(query)) ||
        (p.description_tr && p.description_tr.toLowerCase().includes(query))
      )
    }
  ...
  return (
    <main className="bg-white min-h-screen">
      ...
      <section id="products-section" className="max-w-[1400px] mx-auto px-6 py-8">
